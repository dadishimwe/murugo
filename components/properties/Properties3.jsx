...
export default function Properties3() {
  ...
  const { price, size, rooms, bedrooms, bathrooms, type, features, filtered, sortingOption, sorted, currentPage, itemPerPage } = state;
  ...
  useEffect(() => {
    ...
    if (rooms !== "All") {
      const filteredByRooms = properties.filter((elm) => rooms === elm.rooms?.toString());
      filteredArrays = [...filteredArrays, filteredByRooms];
    }
    if (bedrooms !== "All") {
      const filteredByBedrooms = properties.filter((elm) => bedrooms === elm.beds?.toString());
      filteredArrays = [...filteredArrays, filteredByBedrooms];
    }
    if (bathrooms !== "All") {
      const filteredByBathrooms = properties.filter((elm) => bathrooms === elm.baths?.toString());
      filteredArrays = [...filteredArrays, filteredByBathrooms];
    }
    if (type !== "All") {
      const filteredByType = properties.filter((elm) => elm.type?.includes(type));
      filteredArrays = [...filteredArrays, filteredByType];
    }
    const filteredByPrice = properties.filter((elm) => elm.price >= price[0] && elm.price <= price[1]);
    filteredArrays = [...filteredArrays, filteredByPrice];
    const filteredBySize = properties.filter((elm) => elm.sqft >= size[0] && elm.sqft <= size[1]);
    filteredArrays = [...filteredArrays, filteredBySize];

    const commonItems = properties.filter((item) =>
      filteredArrays.length ? filteredArrays.every((array) => array.includes(item)) : true
    );
    dispatch({ type: "SET_FILTERED", payload: commonItems });
  }, [price, size, rooms, bedrooms, bathrooms, type, features, properties]);

  useEffect(() => {
    if (sortingOption === "Price Ascending") {
      dispatch({ type: "SET_SORTED", payload: [...filtered].sort((a, b) => a.price - b.price) });
    } else if (sortingOption === "Price Descending") {
      dispatch({ type: "SET_SORTED", payload: [...filtered].sort((a, b) => b.price - a.price) });
    } else {
  ...
  const allProps = {
    ...
    setBedrooms: (newBedrooms) => dispatch({ type: "SET_BEDROOMS", payload: newBedrooms }),
    setBathrooms: (newBathrooms) => dispatch({ type: "SET_BATHROOMS", payload: newBathrooms }),
    type,
              ...
              <form onSubmit={(e) => e.preventDefault()}>
                <div className="wd-find-select shadow-3">
                  ...
                  <div className="box-btn-advanced">
                    ...
                    <button type="submit" className="tf-btn btn-search primary" href="#">
                      Search <i className="icon icon-search" />
                    </button>
                  </div>
                </div>
                <div ref={ddContainer} className="wd-search-form">
                  <AdvanceSearch2 clearFilter={clearFilter} allProps={allProps} />
                </div>
              </form>
            </div>
          </div>
        ...
        <div className="container">
          <div className="box-title-listing">
            <div className="box-left">
              ...
              <p className="text">There are currently {sorted.length} properties.</p>
            </div>
            <div className="box-filter-tab">
              ...
              <DropdownSelect
                onChange={(value) => {
                  ...
                  if (match) {
                    dispatch({ type: "SET_ITEM_PER_PAGE", payload: parseInt(match[0], 10) });
                    dispatch({ type: "SET_CURRENT_PAGE", payload: 1 });
                  }
                }}
          ...
          <div className="flat-animate-tab">
            <div className="tab-content">
              <div className="tab-pane active show" id="gridLayout" role="tabpanel">
                <div className="row">
                  {sorted.slice((currentPage - 1) * itemPerPage, currentPage * itemPerPage).map((elm) => (
                    <div key={elm._id} className="col-xl-4 col-lg-6 col-md-6">
                      <div className="homelengo-box">
                        <div className="archive-top">
                          <Link href={`/property-details-v1/${elm._id}`} className="images-group">
                            <div className="images-style">
                              <Image className="lazyload" alt="img" src={elm.images[0] || "/default-property.jpg"} width={615} height={405} />
                            </div>
                            <div className="top">
                        ...
                        <div className="archive-bottom">
                          <div className="content-top">
                            <h6 className="text-capitalize">
                              <Link href={`/property-details-v1/${elm._id}`} className="link">{elm.title}</Link>
                            </h6>
                            <ul className="meta-list">
                              <li className="item">
                                <i className="icon icon-bed" />
                                <span className="text-variant-1">Beds:</span>
                                <span className="fw-6">{elm.beds || "N/A"}</span>
                              </li>
                              <li className="item">
                                <i className="icon icon-bath" />
                                <span className="text-variant-1">Baths:</span>
                                <span className="fw-6">{elm.baths || "N/A"}</span>
                              </li>
                              <li className="item">
                                <i className="icon icon-sqft" />
                                <span className="text-variant-1">Sqft:</span>
                                <span className="fw-6">{elm.sqft || "N/A"}</span>
                              </li>
                            </ul>
                          </div>
                          <div className="content-bottom">
                            <div className="d-flex gap-8 align-items-center">
                              <div className="avatar avt-40 round">
                                <Image alt="avt" src={elm.avatar || "/default-avatar.jpg"} width={34} height={34} />
                              </div>
                              <span>{elm.agent || "Unknown Agent"}</span>
                            </div>
                ...
                <ul className="justify-content-center wd-navigation mt-20">
                  <Pagination
                    ...
                    setPage={(value) => dispatch({ type: "SET_CURRENT_PAGE", payload: value })}
              ...
              <div className="tab-pane" id="listLayout" role="tabpanel">
                <div className="row">
                  {sorted.slice((currentPage - 1) * itemPerPage, currentPage * itemPerPage).map((elm) => (
                    <div key={elm._id} className="col-md-6">
                      <div className="homelengo-box list-style-1 line">
                        <div className="archive-top">
                          <Link href={`/property-details-v1/${elm._id}`} className="images-group">
                            <div className="images-style">
                              <Image className="lazyload" alt="img-property" src={elm.images[0] || "/default-property.jpg"} width={344} height={315} />
                            </div>
                            <div className="top">
                        ...
                        <div className="archive-bottom">
                          <div className="content-top">
                            <h6 className="text-capitalize">
                              <Link href={`/property-details-v1/${elm._id}`} className="link text-line-clamp-1">{elm.title}</Link>
                            </h6>
                            <ul className="meta-list">
                              <li className="item">
                                <i className="icon icon-bed" />
                            ...
                            <div className="location">
                              ...
                              <span className="text-line-clamp-1">{elm.address || "Unknown Location"}</span>
                            </div>
                          </div>
                          <div className="content-bottom">
                            <div className="d-flex gap-8 align-items-center">
